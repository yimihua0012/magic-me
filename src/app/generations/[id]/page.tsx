...
export default function GenerationInfoPage() {
    ...
    const loadGeneration = async () => {
      try {
        ...
        if (!session?.access_token) {
          router.push(`/login?returnTo=/generations/${generationId}`)
          return
        }
      ...
      <main className="pt-24 pb-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <Link href="/dashboard" className="mb-3 inline-flex items-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-700">
                <ArrowLeft className="h-4 w-4" />
                Back to Dashboard
              </Link>
              <h1 className="text-3xl font-bold text-slate-900">Headshot Details</h1>
              <p className="mt-1 text-slate-600">
                {generation ? `${photos.length} images generated on ${formatDate(generation.created_at)}` : 'Review and download your generated images.'}
              </p>
            </div>
            <Link href="/upload">
          ...
          {isLoading ? (
            <Card className="p-12 text-center">
              ...
              <p className="text-slate-600">Loading generation...</p>
            </Card>
          ) : error ? (
            <Card className="p-12 text-center">
            ...
            <Card className="p-12 text-center">
              ...
              <h2 className="mb-2 text-xl font-semibold text-slate-900">No images available</h2>
              <p className="mb-6 text-slate-600">This generation has not produced images yet.</p>
              <Link href="/dashboard">
                <Button variant="secondary">Back to Dashboard</Button>
              </Link>
            </Card>
            ...
            <>
              <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm text-slate-500">{selectedPhotos.size} selected</span>
                </div>
                <Button
                  variant="secondary"
                  size="sm"
                ...
                {photos.map((photo, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      aria-label={selectedPhotos.has(index) ? 'Deselect image' : 'Select image'}
                    >
