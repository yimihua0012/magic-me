...
assert(read('src/app/api/admin/blog-post-draft/route.ts').includes('exactly one localized long-tail Google search keyword'), 'DeepSeek preparation must return exactly one localized long-tail Google search keyword')
assert(read('src/app/api/admin/blog-post-draft/route.ts').includes('Draft angle: avatar and headshot style transformation'), 'DeepSeek preparation must include varied draft angles')
...
assert(pricingJsonLd.includes("'@graph'"), 'pricing JSON-LD must group the main Product and plan variants')
assert(pricingJsonLd.includes('digitalDeliveryPolicy(currency)'), 'pricing JSON-LD must define delivery policy once per page')
assert(pricingJsonLd.includes('hasMerchantReturnPolicy: returnPolicy'), 'pricing offers must include hasMerchantReturnPolicy')
assert(pricingJsonLd.includes('shippingDetails'), 'pricing offers must include shippingDetails')
...
assert(merchantStructuredData.includes('hasMerchantReturnPolicy: merchantReturnPolicy()'), 'merchant offers must inline return policy for Google product checks')
assert(merchantStructuredData.includes('shippingDetails: digitalDeliveryPolicy(currency)'), 'merchant offers must inline shipping details for Google product checks')
